...
from utils.twitter_api import TwitterApi, API_ENDPOINT
...
class Tp2QueryTwitter(Tp2Query):
    ...
    def _frament_fill_meta(self, tpq, fragment, last_result, total_nb_triples, nb_triple_per_page, request, tpf_url):
        ...
        fragment.add_meta_quad(source, DCTERMS['description'], Literal("Triples from the twitter api matching the pattern {?s=%s, ?p=%s, ?o=%s}" % (tpq.subject, tpq.predicate, tpq.obj)), meta_graph)
        ...
        fragment.add_meta_quad(source, PROV['wasDerivedFrom'], URIRef(API_ENDPOINT), meta_graph)
